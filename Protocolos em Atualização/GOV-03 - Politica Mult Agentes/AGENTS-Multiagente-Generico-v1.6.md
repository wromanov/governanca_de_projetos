# Política transversal de execução direta e delegação multiagente — v1.6
...
## 8. Papéis de subagentes

Papéis são ferramentas de decomposição, não participantes obrigatórios.

Exemplos:

- `explorer` — estrutura, arquivos, símbolos, dependências e fluxo;
- `test_analyst` — testes, cobertura, regressões e boundaries;
- `worker` — implementação clara e previamente decidida;
- `reviewer` — revisão independente quando materialmente justificada;
- `security_reviewer` — superfície de segurança relevante;
- `architect` — consulta arquitetural excepcional.

```text
ROLE_EXISTS != ROLE_MUST_BE_USED
```

Não criar um subagente apenas porque existe um perfil disponível para ele.
...
Não crie `worker` apenas para transferir código de um agente para outro.

Um `worker` só é adequado quando houver ganho real, como:
...
Não criar reviewer automaticamente porque houve implementação.
...
Use especialista/reviewer de segurança somente quando existir superfície
material, como:
